...
from api import ParseCLIOutput, RunDeviceCommand, RunCommand
...
@app.post('/do_command')
async def DoCommandEndpoint(host: str = Form(), password: str = Form(), secret: str = Form(), device_type: str = Form(), command: str = Form(), enable_mode: bool = Form()):
    return RunCommand(host, password, secret, device_type, command, enable_mode)
